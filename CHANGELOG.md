...
- The Clay themes are generated by `tools/generate-clay.mjs` from the colour maps in `tools/clay-map.json` rather than maintained by hand. Eight themes sharing palettes in pairs drift apart one edit at a time otherwise — a fix applied to Dark and forgotten on Dark Clay is the kind of thing nobody notices for months.
